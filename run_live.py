...
from ai_analysis import JevReviewer, gate_decision, gate_mode_from_env, idle_review, reviewer_from_env, run_jev_forever
...
def serve_dashboard(environment: str, gate_mode: str, model: str) -> None:
    def review(_snapshot) -> dict:
        return STORE.latest_jev() or idle_review("Waiting for the first five-second Jev review.", model)

    app = create_app(STORE, review, environment, gate_mode)
...
def start_auto_trading(host: str, port: int, symbol: str, gate_mode: str, reviewer: JevReviewer) -> str:
    ...
    max_gate_age = reviewer.settings.max_gate_age_seconds
    entry_gate = (lambda snapshot, now: gate_decision(STORE.latest_jev(), snapshot, now, max_gate_age)) if gate_mode == "enforce" else None
...
def main() -> None:
    ...
    reviewer = reviewer_from_env(os.environ)
    feed = MoomooFeed(host, port)
    feed.connect([symbol, benchmark])
    environment = start_auto_trading(host, port, symbol, gate_mode, reviewer) if auto_trade else "OFF"
    threading.Thread(target=run_jev_forever, args=(STORE, JEV_INTERVAL_SECONDS, reviewer), name="jev", daemon=True).start()
    threading.Thread(target=serve_dashboard, args=(environment, gate_mode, reviewer.settings.model), name="dashboard", daemon=True).start()
    threading.Thread(
